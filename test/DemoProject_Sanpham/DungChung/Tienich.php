...
function SendMail($email,$random_number){   
    ...
    try {
        ...
        $mail->Username = 'nc.hieu2512@gmail.com'; // SMTP username
        $mail->Password = 'rgekyuittpdphwlh';   // SMTP password
        $mail->SMTPSecure = 'ssl';  // encryption TLS/SSL 
        $mail->Port = 465;  // port to connect to                
        $mail->setFrom('nc.hieu2512@gmail.com', 'Group1' ); 
...
function SendMail2($email,$link){   
    ...
    try {
        ...
        $mail->isHTML(true);  // Set email format to HTML
        $mail->Subject = 'Xác nhận Email';
        $noidungthu = 'Link xác nhận của bạn là: ' . $link ; 
        $mail->Body = $noidungthu;
        $mail->smtpConnect( array(
            "ssl" => array(
                "verify_peer" => false,
